...
3.d. The "agregate" key is an optional object. The AI should include it only when a chart type has a large number of categories (e.g., > 10) on the x-axis. This object must contain:
    - "strategy": string (e.g., "top_n")
    - "n": number (e.g., 5, 10)
    - "othersLabel": string (e.g., "Lainnya" or "Other Categories")
...
- If the dataset has more than 5 unique categories, the AI should include the "agregate" key with "n": 5 and "othersLabel": "Lainnya".
- When filtering string properties, the AI should use a case-insensitive check by converting both strings to lowercase.
11. Instructions for chartType = barChart:
- If the x axis is of type string, if the number of unique x values is between 11 and 50, the AI should include the "agregate" key with "n": 10 and "othersLabel": "Lainnya".
...
- If the dataset has more than 10 unique categories, the AI should include the "agregate" key with "n": 10 and "othersLabel": "Lainnya".
- When filtering string properties, the AI should use a case-insensitive check by converting both strings to lowercase.
`;
