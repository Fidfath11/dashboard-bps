...
export function Dashboard(
  ...
  const [shuffledCharts, setShuffledCharts] = React.useState(props.dashboard.charts);
      ...
      <SimpleGrid columns={{ base: 1 }} spacing={6} mb={6}>
        {shuffledCharts.map((chart, index) => (
          <Flex
            ...
            maxH="450px"
            overflowX={
              chart.chartType === "barChart" || chart.chartType === "lineChart" || chart.chartType === "treemapChart"
                ? "auto"
                : "hidden"
            }
            overflowY="hidden"
          >
            ...
            <Box flexGrow={1} position="relative">
                {chart.chartType === "lineChart" && (
                  <LineChart config={chart} data={filteredData} zoomLevel={chartZoomLevels[chart.title]} />
                )}
                {chart.chartType === "barChart" && (
                  <BarChart config={chart} data={filteredData} zoomLevel={chartZoomLevels[chart.title]} />
                )}
